export class ControllableForm extends HTMLElement {
    ...
    get innerRequired() {
        return this.hasAttribute("inner-required");
    }

    set innerRequired(newValue) {
        if (newValue) {
            this.setAttribute("inner-required", "");
        } else {
            this.removeAttribute("inner-required");
        }
    }
    ...
    render() {
        const inner_value = this.getAttribute("inner-value") || "";
        const inner_required = this.hasAttribute("inner-required") ? "required" : "";

        this.innerHTML = `
            <form>
                <label>
                    <span></span>
                    <input name="inner" value="${inner_value}" ${inner_required} disabled>
                </label>
